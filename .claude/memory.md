...
- Rich shell integration now handles stale idle prompt input by bypassing `runCommand` and sending the command via guarded `sendText`, instead of letting `runCommand` inject `Ctrl+C` at an idle prompt.
- A later manual smoke still showed `Get-Location` returning only `^C`; root cause was a second Rich-path Ctrl+C branch inside `TerminalInstance.runCommand()` after its own prompt-readiness wait. Rich sync tool execution now bypasses `runCommand()` entirely, waits locally for prompt readiness/idle, assigns the next command id through shell integration metadata, and sends the command with guarded `sendText`.
- Rich strategy now receives the tool-initiated input guard too, so its own `sendText(...)` calls do not count as manual user input.
- Follow-up smoke showed `Ctrl+U` can be echoed as a literal `^U` prefix in the user's PowerShell terminal and corrupt every command. Rich execution now avoids both `Ctrl+C` and `Ctrl+U`; stale Rich prompt input bypasses `runCommand()` but sends no control character before the command.
- Repackaged after the no-control-character Rich fix:
  - `artifacts/out/insider/win32-x64/system-setup/Director-CodeSetup-x64-1.120.0.exe`, sha256 `2567E75C435F5E9561C3E86408217E7B7943C9762725A809F856982ECD6F7E3B`
  - `artifacts/out/insider/win32-x64/user-setup/Director-CodeUserSetup-x64-1.120.0.exe`, sha256 `B15D7E4491DE9B3CD3653B7D86F0F9425FD4270803067D03B2D0244FC033FE8D`
